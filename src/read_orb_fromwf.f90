...
subroutine read_orb_fromwf
  ...
  implicit none
  integer isf,i,nds,ir,color_ir,tag,rank0,isp,ic,ip,st
  character*9             :: ch
  real*8                  :: occ
  ...
  do isf=1,orb_indx
     ...
     rnk0: if(rank==0) then
        ...
        call read_ur
        if(isf==orb_indx) call nrmlz(ur)
     end if rnk0
  end do
  call bcast_r8(ur,size(ur),rank0)

  if(.not.allocated(orb_rd)) then; allocate(orb_rd(n),stat=st); call check0(st,' orb_rd ')
  endif
...
end subroutine read_orb_fromwf
