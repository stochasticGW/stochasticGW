...
!!! END PTMOD
  use ppm,   only : rrpp, vpploc, phipp, vlpp !, vphipp
...
!!! PT Hamann has below, currently this is in kb_alloc.f90
!  allocate( ngs(                           na), stat=st);  call check0(st,' ngs ')
!  allocate(racut_a(       matop), stat=st); call check0(st,' racut_a ')
! Also, kb_alloc.f90 has 'iub', 'iut', 'pvp' 'vp', not in Hamann
!!! END PT
...
!!! PT these four arrays are in Hamann vsn
  allocate( lpp_m(              nproj_max,matop), stat=st);  call check0(st,' lpp        ')
  allocate( nproj_m(                      matop), stat=st);  call check0(st,' nproj      ')
  allocate( phipp_m(     nrppmx,nproj_max,matop), stat=st);  call check0(st,' vpploc ')
  allocate( dij_diag_m(         nproj_max,matop), stat=st); call check0(st,' dij_diag   ')
!!!
